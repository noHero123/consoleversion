...
namespace ConsoleApplication1.noidAI
{
    ...
    class PlayRandomGames//intended to be a  pre-"hello world" for MCTS
    {
        ...
        static CardDB.cardIDEnum[] myDeck = new CardDB.cardIDEnum[30];
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                myDeck[i] = (CardDB.cardIDEnum)generator.Next(Enum.GetNames(typeof(CardDB.cardIDEnum)).Length);
            }
            Playfield defaultField = new Playfield();
            // add a card to my hand from my deck, and a murloc tidecaller
            //public Card getCardDataFromID(cardIDEnum id)
            CardDB.Card aCard =  CardDB.Instance.getCardData(CardDB.cardName.murloctidecaller);
            CardDB.Card anotherCard = CardDB.Instance.getCardDataFromID(myDeck[0]);
            Handmanager.Handcard myHandCard = new Handmanager.Handcard(aCard);
            Handmanager.Handcard mySecondHandCard = new Handmanager.Handcard(anotherCard);
            List<Handmanager.Handcard> forFun = new List<Handmanager.Handcard>();
            defaultField.owncards = forFun;
            
            // is this how I would set my mana?

            defaultField.mana = 10;
            defaultField.ownMaxMana = 10;
            
            // this doesnt appear to add anything as a action...
            CardDB.Card ashbringer = CardDB.Instance.getCardData(CardDB.cardName.ashbringer);
            defaultField.equipWeapon(ashbringer,true); // not actually a weapon
            //defaultField.equipWeapon(
    ...
    {
        int deckState;// which card we're on. 
        CardDB.Card[] myDeck; // this will be randomized per play, but not per action (if that makes any sense)
        CardDB.Card[] enemyDeck; // this will be randomized per play, but not per action (if that makes any sense)
...
// some helpful tidbits as i go along.....
// List<Action> actions = movegen.getMoveList(<playfield>, <lethalcheck>, <penaltymanager>, <cuttting targets>);
// public class Hrtprozis
//    { has a method for returning correct enum for hero power i think
